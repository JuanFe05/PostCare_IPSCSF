  ...
  return (
    <div className="flex items-center justify-between mt-6 px-4 py-3 bg-gray-50 border-t border-gray-200 rounded-b-lg">
      <span className="text-sm text-gray-700">
        ...
        <span className="font-semibold">{dataLength}</span> servicios
      </span>
      <div className="flex items-center gap-2">
        ...
        <span className="px-4 py-2 text-sm font-medium text-gray-700">Página <span className="font-bold">{pageIndex + 1}</span> de <span className="font-bold">{pageOptions.length}</span></span>
        <button onClick={() => nextPage()} disabled={!canNextPage} className={`px-4 py-2 rounded-md border text-sm font-medium transition-colors ${!canNextPage ? 'bg-gray-100 text-gray-400 border-gray-300 cursor-not-allowed' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50 cursor-pointer'}`}>Siguiente</button>
        <button onClick={() => gotoPage(pageOptions.length - 1)} disabled={!canNextPage} className={`px-3 py-2 rounded-md border text-sm font-medium transition-colors ${!canNextPage ? 'bg-gray-100 text-gray-400 border-gray-300 cursor-not-allowed' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50 cursor-pointer'}`} title="Última página">»»</button>
      </div>
    </div>
  );
}
